...
export interface SectionFounderProps {
  className?: string;
}

const SectionFounder: FC<SectionFounderProps> = ({ className = "" }) => {
  ...
  return (
    <div className={`nc-SectionFounder relative ${className}`}>
      <Heading
      ...
      <div className="grid sm:grid-cols-2 gap-x-5 gap-y-8 lg:grid-cols-4 xl:gap-x-8">
        {FOUNDER_DEMO.map((item) => (
          <div key={item.id} className="max-w-sm" data-aos="fade-up" data-aos-delay={parseInt(item.id) * 100}>
